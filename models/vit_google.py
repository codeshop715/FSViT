...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        attenpool = attention_probs.mean(dim=1)
        attenpool = attenpool.sum(dim=1)
        ...
        return attention_output, weights, attenpool
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x, weights, attenpool = self.attn(x)
        x = x + h
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            hidden_states, weights, attenpool = layer_block(hidden_states)
            if self.vis:
                attn_weights.append(weights)
        encoded = self.encoder_norm(hidden_states)
        return encoded, attn_weights, attenpool
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded, attn_weights, attenpool = self.encoder(embedding_output)
        return encoded, attn_weights, attenpool
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None, use_patches=True):
        x, attn_weights, attenpool = self.transformer(x)
